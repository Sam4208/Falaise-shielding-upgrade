...
namespace snemo {
  ...
  namespace processing {
	...
	_pcd2cd_tracker_height_error_single_ts_a_ = fps.get<falaise::length_t>("tracker_height_error_single_ts_a", {3.0, "cm"})();
	_pcd2cd_tracker_height_error_single_ts_b_ = fps.get<falaise::length_t>("tracker_height_error_single_ts_b", {3.0, "cm"})();
	...
	const double H = _pcd2cd_tracker_height_effective_;
	const double H0 = _pcd2cd_tracker_height_offset_;
	const double error_single_ts_a = _pcd2cd_tracker_height_error_single_ts_a_;
	const double error_single_ts_b = _pcd2cd_tracker_height_error_single_ts_b_;
	...
	if (has_both_cathode) {
	  ...
	  const double z_abs = z_norm * H  + H0; 
	  double z_error_theory = sqrt((pow(error_single_ts_a,2)*pow(1+z_norm,2))+(pow(error_single_ts_b,2)*(1+z_norm)));
	  double z_error_limit = sqrt((pow(error_single_ts_a,2)*pow(1,2))+(pow(error_single_ts_b,2)*(1)))*pow(3,-1);
	  const double z_error = std::max(z_error_theory, z_error_limit);
	  if (z_norm<0.5){
	  cd_tracker_hit_.set_z(z_abs);
	  cd_tracker_hit_.set_sigma_z(z_error);}
